...
| financial | [Wandb] | 数据量少（7.3K），数值推理类答案 EM 匹配困难 |
| science | [Wandb] | 数据量适中（9.7K），多选形式 |
...
| 指标 | 单域 searchr1 (Step 400) | 单域 multistep (Step 400) | 多域 | 说明 |
|------|------------------------|--------------------------|------|------|
| Overall EM | 33.76% | 37.34% | [Wandb] | 不可直接对比（数据集不同） |
| 搜索行为 | avg 1.12 | avg 1.00 | [Wandb] | — |
| 格式合规 | 99.92% | 100% | [Wandb] | — |
